...
function Editor() {
    ...
    function editItem(item) {
        console.log(item);
        fetch('http://localhost:3333/goods/' + item.id, {
            method: "PATCH",
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(item)
        })
        ...
        <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap' }}>{
                dataList.length > 0 ? dataList.slice(page * pageLength - pageLength, pageLength * page).map((item, index) => {
                    if (item.caption.slice(0, inpValue.length).toLowerCase() == inpValue.toLowerCase()) return <CardItem key={index+"-Editor"} crud={[deleteItem, editItem]} data={item} key={item.id + '-GameItem'} />
                }) : ''
